    ...
    internal abstract class ArithmeticLiteral
    {
        ...
        private static Dictionary<Type, LiteralMaker> CreateTypesDictionary()
        {
            ...
            Dictionary<Type, LiteralMaker> dictionary = new Dictionary<Type, LiteralMaker>(16)
            {
                ...
                { typeof(char), MakeChar },
                { typeof(short), MakeShort },
                { typeof(int), MakeInt },
                { typeof(long), MakeLong },
                { typeof(ushort), MakeUShort },
                { typeof(uint), MakeUInt },
                { typeof(ulong), MakeULong },
                { typeof(float), MakeFloat },
                { typeof(double), MakeDouble },
                { typeof(decimal), MakeDecimal },
                { typeof(bool), MakeBoolean },
                { typeof(string), MakeString },
        ...
        internal static ArithmeticLiteral MakeLiteral(Type literalType, object literalValue)
        {
            ...
            return (types.TryGetValue(literalType, out LiteralMaker f)) ? f(literalValue) : null;
        }
